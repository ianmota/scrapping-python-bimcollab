from time import sleep, time
from tkinter import *
from tkinter import filedialog
from threading import *
from tkinter.scrolledtext import ScrolledText
from backend.scrapper.scrapper import ScrapperColect, ScrapperResearch
import base64
from data.dataSave import *
from data.dataAnalysys import *

class Application(ScrapperResearch,ScrapperColect,Analysys):
    
    def __init__(self):
        self.root = Tk()
        self.str_LOCAL = StringVar()
        self.str_COLETA = StringVar()
        self.Imagens()
        self.Root01()
        self.Root01Labels()
        self.Root01Button()
        self.Root01Entry()
        self.LogConfig()
        self.root.mainloop()
        
    def Root01(self):
        self.root.title("Tabela de incompatibilidades")
        self.root.configure(background='#71A67a')
        self.root.geometry('600x300')
        self.root.resizable(False,False)
        self.root.protocol("WM_DELETE_WINDOW", self._close)
 
        self.logoP5D = PhotoImage(data=base64.b64decode(self.logo))
        self.root.iconphoto(False,self.logoP5D)
            
    def Root01Labels(self):
        lb_width = 100
        self.lb_usuario = Label(self.root, text="Usuário", font=('verdana',11,'bold'),justify='center')
        self.lb_usuario.place(x=5,y=150,width=lb_width)
        
        self.lb_senha = Label(self.root, text="Senha", font=('verdana',11,'bold'))
        self.lb_senha.place(x=5,y=180,width=lb_width)
        
        self.lb_local = Label(self.root, text="Local", font=('verdana',11,'bold'))
        self.lb_local.place(x=5,y=210,width=lb_width)
        
        textoImpacto = 'Se você não tem dúvida é porque está mal \ninformado'
        
        self.lb_fraseImpacto = ScrolledText(self.root, font=('verdana',12,'bold'),state="disabled")
        self.lb_fraseImpacto.place(x=160, y=10, width=430, height=130)
        
        self.lb_fraseImpacto["state"] = "normal"
        self.lb_fraseImpacto.insert(1.0,textoImpacto)
        self.lb_fraseImpacto["state"] = "disabled"
        
        self.fr_logoP5D = Label(self.root, image= self.logoP5D)
        self.fr_logoP5D.place(x=5,y=10,height=130,width=140)

    def Root01Entry(self):
        en_width = 480
        self.en_usuario = Entry(self.root,font=('verdana',10))
        self.en_usuario.place(x=110,y=150,width=en_width, )

        self.en_senha = Entry(self.root,font=('verdana',10),show='*')
        self.en_senha.place(x=110,y=180,width=en_width)
    
        self.en_LOCAL = Entry(self.root,font=('verdana',10),state="disabled",textvariable=self.str_LOCAL)
        self.en_LOCAL.place(x=110,y=210,width=375)
        
    def Root01Button(self):    
        self.bt_confirmar = Button(self.root, text="Confirmar", font= ('verdana',11,'bold'),command=self.ErrorVerify)
        self.bt_confirmar.place(relx=0.4, rely=0.85)
        
        self.bt_localSave = Button(self.root, font= ('verdana',11,'bold'), justify='center',command=self._get_folder,text="Selecionar")
        self.bt_localSave.place(x=490, y=210, width=100, height=26)
    def ErrorVerify(self):
        if self.en_senha.get() and self.en_usuario.get():
            try: 
                self.en_localSalve
            except AttributeError:
                self.lb_fraseImpacto["state"]="normal"
                self.lb_fraseImpacto.delete(1.0,END)
                self.lb_fraseImpacto.insert(END,"ADICIONAR LOCAL DO ARQUIVO --> ERRO!","red")
                self.lb_fraseImpacto["state"]="disabled"
            else:
                self._th_get_html()
                
        else:
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,"ADICIONAR USUARIO E SENHA --> ERRO!","red")
            self.lb_fraseImpacto["state"]="disabled"
            
    def _th_get_html(self):
        self.t1 = Thread(target=self._get_html)
        self.t1.start()
        
    def _close(self):
        try:
            self.t1.join()
        except AttributeError:
            pass
            
        self.root.destroy()
        
    def _get_html(self):
        self.bt_confirmar["state"]="disabled"
        self.en_usuario["state"]="disabled"
        self.en_senha["state"] = "disabled"
        self.bt_localSave["state"] = "disabled"
        
        email = str(self.en_usuario.get())
        password = str(self.en_senha.get())

        self.lb_fraseImpacto["state"]="normal"
        self.lb_fraseImpacto.delete(1.0,END)
        self.lb_fraseImpacto.insert(END,"ABRINDO O SERVIDOR","gray")
        self.lb_fraseImpacto["state"]="disabled"
        
        self.browser = ScrapperResearch(email,password)
        try:
            self.browser.openResearch()
            self.browser.addLogin()
            
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.insert(END,"\nFAZENDO LOGIN","gray")
            self.lb_fraseImpacto["state"]="disabled"

            
            self.browser.addSenha()
            self.browser.addCompany()
            
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.insert(END,"\nLOGIN --> OK","green")
            self.lb_fraseImpacto["state"]="disabled"
            
            self.browser.selectProject()
            self.browser.goToIssues()
            self.browser.openAll()
            
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,"COLETANDO AS INCOMPATIBILIDADES","green")
            self.lb_fraseImpacto["state"]="disabled"
            
        except:   
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,"ABERTURA DO SERVIDOR --> ERRO!","red")
            self.lb_fraseImpacto["state"]="disabled"

        try:
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,f"COLETANDO OS DADOS, AGUARDE\n","grey")
            self.lb_fraseImpacto["state"]="disabled"
            self.str_COLETA.set(self.browser.statusColeta)
            self.browser.htmlGenerator()
            
        except:
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,"COLETA DE DADOS --> ERRO!","red")
            self.lb_fraseImpacto["state"]="disabled"
            
        else:
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.insert(END,"\nCOLETA DE DADOS --> OK \
                                        \nPLANILHA PRONTA","green")
            self.lb_fraseImpacto["state"]="disabled"
        
        self._create_dictionary()
        
    def _get_folder(self):
        self.en_localSalve = filedialog.askdirectory()
        
        self.str_LOCAL.set(self.en_localSalve)

    
    def _create_dictionary(self):
        id = []
        titulo = []
        status = []
        descricao = []
        empreendimento = []
        etiqueta = []
        responsavel = []
        fase = []
        tipo = []
        area = []
        prazo = []
        prioridade =[]
        ultimaAlteracao = []
        primeiraAlteracao = []

        try:
            self.dicionario = {}
            for i in self.browser.htmlList:
                coleta = ScrapperColect(i)
                dicBase = coleta.dictionaryGeneration()
                
                id.append(dicBase.get("ID"))
                titulo.append(dicBase.get("Título"))    
                status.append(dicBase.get("Status")) 
                descricao.append(dicBase.get("Descrição")) 
                empreendimento.append(dicBase.get("Empreendimento"))
                etiqueta.append(dicBase.get("Etiqueta"))
                responsavel.append(dicBase.get("Responsável"))
                fase.append(dicBase.get("Fase"))
                tipo.append(dicBase.get("Tipo"))
                area.append(dicBase.get("Local"))
                prazo.append(dicBase.get("Prazo"))
                prioridade.append(dicBase.get("Prioridade"))
                ultimaAlteracao.append(dicBase.get("Última modificação"))
                primeiraAlteracao.append(dicBase.get("Primeira alteração"))
                
                
            self.dicionario["ID"] = id
            self.dicionario["Título"] = titulo
            self.dicionario["Status"] = status
            self.dicionario["Descrição"] = descricao
            self.dicionario["Empreendimento"] = empreendimento
            self.dicionario["Etiqueta"] = etiqueta
            self.dicionario["Responsável"] = responsavel
            self.dicionario["Fase"] = fase
            self.dicionario["Tipo"] = tipo
            self.dicionario["Local"] = area
            self.dicionario["Prazo"] = prazo
            self.dicionario["Prioridade"] = prioridade
            self.dicionario["Última alteração"] = ultimaAlteracao
            self.dicionario["Primeira alteração"] = primeiraAlteracao
            
            self.FilterDict()
            
        except UnboundLocalError:
            self.bt_confirmar["state"]="normal"
            self.en_usuario["state"]="normal"
            self.en_senha["state"] = "normal"
            self.bt_localSave["state"] = "normal"
            
            self.lb_fraseImpacto["state"]="normal"
            self.lb_fraseImpacto.delete(1.0,END)
            self.lb_fraseImpacto.insert(END,"ERRO GRAVE!\nVERIFICAR LOGIN E SENHA","red")
            self.lb_fraseImpacto["state"]="disabled"

            
    def FilterDict(self):
        dicfilter = Analysys(self.dicionario)
        dicfilter.Filter()
        
        self.filt_dict = dicfilter.DicFiltered()
        self._save()
        
    def _save(self):
        nomeEmail = self.en_usuario.get()
        nomeArquivo = nomeEmail.split('@')
        
        dbBuildCSV(self.dicionario,f'{self.en_localSalve}/{nomeArquivo[0]}.csv')
        dbBuildCSV(self.filt_dict,f'{self.en_localSalve}/{nomeArquivo[0]}_filt.csv')
        
        self.bt_confirmar["state"]="normal"
        self.en_usuario["state"]="normal"
        self.en_senha["state"] = "normal"
        self.bt_localSave["state"] = "normal"

    def LogConfig(self):
        self.lb_fraseImpacto.tag_config("red",foreground="red")
        self.lb_fraseImpacto.tag_config("gray",foreground="gray")
        self.lb_fraseImpacto.tag_config("green",foreground="green")
        
    def Imagens(self):
        self.logo = ""